...
**L'orario di passaggio si prende dal profilo delle corse, non dal bucket della fermata.** Il profilo dice a quanti secondi dalla partenza il mezzo si trova a un dato metro, e le partenze sono elencate: l'orario è già in memoria. Chiedere invece il bucket funziona ma è razionato a due richieste per volta, e con trecento mezzi che ne vogliono uno ciascuno l'ancoraggio si attiverebbe su pochi e a caso — la stessa lotteria che aveva tenuto spenta la compensazione. Il profilo copre **602 varianti su 894**; per le altre resta il bucket come ripiego, e la scheda dichiara quando nemmeno quello è arrivato.
...
**La direzione della freccia viene dal progresso lungo il percorso, non dalla riproiezione del punto.** Sono due cose diverse e la seconda sembrava equivalente. La riproiezione non ha memoria: su una shape che si sovrappone a sé stessa — capolinea ad anello, andata e ritorno sulla stessa via — ritrova il mezzo sul passaggio sbagliato, e la freccia si gira di 180°. Misurato sulle 894 shape GTT con rumore GPS realistico, contro la direzione realmente tenuta nei 125 metri successivi:

| da dove viene la direzione | mediana | p90 | p99 | oltre 45° | oltre 90° |
| --- | --- | --- | --- | --- | --- |
| dal singolo segmento | 0,3° | 33,4° | 173,5° | 7,4% | 2,5% |
| da una base di 40 m | 1,1° | 30,0° | 173,4° | 6,2% | 2,0% |
| base + memoria del progresso | 1,0° | 26,1° | **71,8°** | 4,6% | **0,5%** |

La base da sola quasi non sposta la coda: il 99° percentile resta a 173°, cioè freccia rovesciata. È la memoria a toglierla, perché il progresso è continuo e non torna indietro (`routeMotion`), quindi non può saltare da un passaggio all'altro della stessa shape. La riproiezione resta per la **posizione**, che deve stare sul tracciato disegnato. Chi tocca `vehiclesToGeoJson` non rimetta `snapped.bearing` davanti a `routeBearings`.

**La freccia disegna una direzione geografica in spazio schermo, quindi va compensata la rotazione della mappa.** `text-rotation-alignment` è `viewport`: il glifo viene ruotato rispetto allo schermo, non alla mappa. Finché la mappa è a nord in alto le due cose coincidono, ma appena ruota ogni freccia sbaglia esattamente della rotazione, mentre le sagome dei mezzi - allineate alla mappa - restano giuste. È il difetto che si vede come «freccia a caso accanto a un mezzo orientato bene». La rotazione viene quindi sottratta in `arrowBearing` mentre si costruisce il GeoJSON. E la mappa non ruota più: `dragRotate: false` toglieva solo la rotazione col mouse, quella a due dita restava attiva e su un telefono scatta da sola durante il pinch, quindi ora c'è anche `touchZoomRotate.disableRotation()`.

**Dirlo una volta sola.** Rendere onesta la scheda di una corsa non accertata ha prodotto il difetto opposto: «non accertata» compariva nel titolo, nel sottotitolo, nel badge della direzione e sopra il riquadro dell'immagine, e le ultime due si sovrapponevano perché sono posizionate in assoluto sopra un pannello diventato più alto. Ora lo dice il titolo; il badge dice **da dove viene la posizione**, non che è prevista; e la scheda tecnica del mezzo **sparisce**, perché su una corsa senza mezzo era una colonna di «n/d». Stessa regola per la velocità: viene dall'orario, quindi la fonte dice «passo dell'orario» invece di «non disponibile» accanto a un numero che c'è.

**Una corsa non accertata non ha un mezzo, e la scheda non deve fingere il contrario.** Ne dichiarava tre di cose non vere: un «ID feed» che il feed non ha mai mandato (l'identificativo se lo inventa l'app per seguire la corsa fra un aggiornamento e l'altro), il render del bus generico, e la fascia «Render verificato». Il render è la cosa che un lettore prende per un'osservazione più di ogni altra. Ora la scheda dice «Corsa <linea> non accertata», «nessun mezzo osservato», e non mostra nessun mezzo. `smoke:map --feed-vuoto` apre la scheda di una corsa non accertata e fallisce se ci ritrova una di quelle tre.
...
- **Sostituendo il contenuto di un render, cambiare anche il nome del file.** Gli asset in `public/` vengono serviti a URL stabile, senza hash: se il nome resta uguale, browser e CDN continuano a servire la versione vecchia e la correzione non si vede. Il workflow `generate-fleet-render` accetta per questo un `variant`: `v7` scrive `<chiave>-gtt-render-v7.webp`. Prima andava fatto a mano, e dimenticarlo significava una correzione che nessuno vedeva.
...
- **Un render che viene da un'immagine si rigenera con quell'immagine allegata, non ridescrivendola.** Il 2026-08-10 il render della serie 5000 in produzione e' stato riconosciuto come inventato da chi vede quelle vetture ogni giorno. Ritirandolo e' venuto fuori che l'immagine giusta era gia' stata nel repository ed era stata sostituita da una **rigenerazione descritta a parole**, che aveva perso tutto quello che rendeva quel tram quel tram. Da qui `referenceAsset` sul cluster: quando c'e', `fleet-render.mjs` passa dall'endpoint delle modifiche e allega l'immagine, e il prompt cambia mestiere - non deve piu' evocare un mezzo, deve elencare cosa non si tocca. Il render attuale della 5000 e' nato cosi', con la foto della 5014 come riferimento; la foto non sta piu' nel repository perche' la sua provenienza non risultava da nessuna parte, e il riferimento per le prossime generazioni e' il render stesso.
- **Le scritte su un render si controllano ingrandite, non a figura intera.** Sulla 5000 sono servite tre generazioni: la prima ha reso il marchio con un glifo che a piena figura sembrava un altro operatore, la seconda ha scritto "G-T-T" con i trattini perche' il prompt le dettava lettera per lettera. Marchio dell'operatore e stemma della citta' sono livrea e vanno chiesti; matricola e numero di linea no, perche' sarebbero l'identita' di una vettura e di una corsa precise che il render non rappresenta.
...
6. **Le shape GTT sono schematiche, non geometria stradale, ed è il limite più visibile della mappa.** Misurato sulle 894 varianti pubblicate: fra un vertice e l'altro corrono **48,8 m di mediana**, 264 m al 90° percentile, fino a 5,8 km; il 43% dei segmenti supera i 60 m. La linea 17 ha 128 vertici per 18,7 km, cioè **un vertice ogni 147 m**. Sulla rotonda di Via Lucio Battisti quella variante ha due soli vertici in 250 metri, con un salto di 199 m: la linea disegnata taglia la rotonda in linea retta, e il mezzo che la segue la taglia con lei.
